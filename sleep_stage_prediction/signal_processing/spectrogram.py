...
def cal_psd_old(x, fs, window, noverlap, nfft, f_min, f_max, plot_flag=True):
    ...
    t1 = time()
    S, f, t, ax = plt.specgram(x=x_, Fs=fs, window=np.blackman(window), NFFT=window, noverlap=noverlap)
...
def cal_psd(x, fs, window, noverlap, nfft, f_min, f_max, f_sub=1, plot_flag=False):
    ...
    f, t, S = spectrogram(x=x_, fs=fs, window=np.blackman(window), nperseg=window, noverlap=noverlap, nfft=nfft)
...
def cal_wavelet(x, fs, window, noverlap, nfft, f_min, f_max, plot_flag=False, sub=16):
    ...
    if plot_flag:
        ...
        plot_cwt(coef_in, t=t, f=frequencies, text=wavelet, x=x[int(start * 32) : int(stop * 32)])
        k = 1
...
def cal_test_spec(x, fs, window, noverlap, nfft, f_min, f_max):
    ...
    plot_spectrogram_all(
        ...
        f_max=4,
        t_min=0,
        t_max=spec.shape[0] // fs,
        text="wavelet_8_mexh",
        x=x[int(start * 32) : int(stop * 32)],
    )
...
def plot_cwt(S, t, f, x=None, text=""):
    ...
    ax.imshow(
        ...
        aspect="auto",
        vmin=np.quantile(S, q=0.05),
        vmax=np.quantile(S, q=0.99),
    )
    ...
    num = 0
    while os.path.isfile("{}\\{}_{}.png".format(save_path, text, num)):
        num += 1
    ...
    if x is not None:
        ...
        ax.plot(t, sig_norm, linewidth=0.4)
        ...
        num = 0
        while os.path.isfile("{}\\{}_{}.png".format(save_path, "sig", num)):
            num += 1
