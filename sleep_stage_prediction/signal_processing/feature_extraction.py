        ...
        feature_dict.update(
            {
                ...
                "q10_{}".format(ext): {"fun": quantile(q=0.10), "ext": ext * fs, "num_feat": 1},
                "q25_{}".format(ext): {"fun": quantile(q=0.25), "ext": ext * fs, "num_feat": 1},
                "q75_{}".format(ext): {"fun": quantile(q=0.75), "ext": ext * fs, "num_feat": 1},
                "q90_{}".format(ext): {"fun": quantile(q=0.90), "ext": ext * fs, "num_feat": 1},
                "VLF_{}".format(ext): {
                    "fun": frequency(window=window * fs, fs=fs, low=0.0033, high=0.04),
                    "ext": ext * fs,
                    "num_feat": 1,
                },
                "LF_{}".format(ext): {
                    "fun": frequency(window=window * fs, fs=fs, low=0.04, high=0.15),
                    "ext": ext * fs,
                    "num_feat": 1,
                },
                "HF_{}".format(ext): {
                    "fun": frequency(window=window * fs, fs=fs, low=0.15, high=0.4),
                    "ext": ext * fs,
                    "num_feat": 1,
                },
            }
...
def FM_features(x, window, fs, window_ext=0):
    feature_dict = {
        "VLF": {
            "fun": frequency(window=window * fs, fs=fs, low=0.0033, high=0.04, normalize_interval=None),
            "ext": 120 * fs,
            "num_feat": 1,
        },
        "LF": {
            "fun": frequency(window=window * fs, fs=fs, low=0.04, high=0.15, normalize_interval=None),
            "ext": 120 * fs,
            "num_feat": 1,
        },
        "HF": {
            "fun": frequency(window=window * fs, fs=fs, low=0.15, high=0.5, normalize_interval=None),
            "ext": 120 * fs,
            "num_feat": 1,
        },
        "LF_HF": {
            "fun": frequency(window=window * fs, fs=fs, low=0.04, high=0.15, normalize_interval=[0.15, 0.5]),
            "ext": 120 * fs,
            "num_feat": 1,
        },
...
def ACC_features(x, window, fs, window_ext=0):
    feature_dict = {
        "LF": {
            "fun": frequency(window=window * fs, fs=fs, low=0.04, high=0.5, normalize_interval=None),
            "ext": 120 * fs,
            "num_feat": 1,
        },
        "HF": {
            "fun": frequency(window=window * fs, fs=fs, low=5, high=14, normalize_interval=None),
            "ext": 120 * fs,
            "num_feat": 1,
        },  # [0.004, 14]
        "mean_energy": {"fun": energy, "ext": 0, "num_feat": 1},
...
def LR(x):
    ...
    HR = 1.0 / (x) * 60  # Must be in bpm.FS dependent!
