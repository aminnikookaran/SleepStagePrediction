...
train, validation, test = get_train_validation_test(data_directory, percent_test=40, percent_validation=20, seed=seed)
...
signals_format = {
    "ACC_merge": {
        ...
        "channel_idx": [0, 1, 2],
        "preprocessing": [
            {"type": "median", "args": {"window_size": 30}},
            {"type": "iqr_normalization_adaptive", "args": {"median_window": 300, "iqr_window": 300}},
            {"type": "clip_by_iqr", "args": {"threshold": 20}},
            {
                "type": "cal_psd",
                "args": {
    ...
    "ppg_signal": {
        ...
        "preprocessing": [
            {"type": "zscore", "args": {}},
            {"type": "change_PPG_direction", "args": {}},
            {"type": "iqr_normalization_adaptive", "args": {"median_window": 301, "iqr_window": 301}},
            {"type": "clip_by_iqr", "args": {"threshold": 20}},
            {
                "type": "cal_psd",
                "args": {
...
train_params = {
    ...
    "loss": {"type": "weighted_loss", "args": {"alpha": 0.35, "gamma": 2}},
    "learning_rate": 1e-4,
    "load_model_name": "",
}
...
def train_execution(experiment, idx=0, load_model_name=""):
    ...
    for dataset in experiment._datasets.keys():
        ...
        unique_validation_recs = set(
            [idx["record"] for idx in train_parameters["validate_dataset"].index_to_record[dataset]]
        )
...
def test_execution(experiment, group="test", load_model_name="", sleep_metrics=False, type="semantic"):
    ...
    model = experiment.get_model(load_model_name=load_model_name)
    train_parameters = experiment.get_train_params()  # TODO - change this to get_params(group)
    thresholds_best, merge_size, min_len = post_parameter_search(
        model=model,
        test_dataset=experiment.get_group_dataset("validation")[
            0
        ],  # TODO - figure out what validation datasets you should use for this
