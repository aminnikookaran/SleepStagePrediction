...
def predict_dataset(model, dataset, threshold, merge_size, min_len, events):

    # get it in the following format:
    # events = [event['name'] for event in datasets.events_format]
    predictions = {record: {event: [] for event in events} for record in dataset.records}
        ...
        for signals, start_indexes in dataset.get_record_batch(record):
            ...
            for prediction_window, start_index in zip(predictions_batch, start_indexes):
                for class_num, event in enumerate(events):
                    ...
                    pp_prediction_binary = post_processing(
                        prediction_binary, merge_size=merge_size[event], min_len=min_len[event]  # post processing
                    )
                    ...
                    for loc in localization_relative:  # only activated if any events present.
                        scores[event] += [
                            np.mean(prediction_window[loc[0] : loc[1], class_num])
                        ]  # find score for each event
                        localizations[event] += [[l * dataset.prediction_resolution + start_index for l in loc]]

        for event in events:
            if localizations[event]:
                predictions_ = non_max_suppression(np.array(localizations[event]), np.array(scores[event]), overlap=0.5)
                    ...
                    for ed_key, ed_it in dataset.events_discard[record].items():
                        if predictions_:
                            overlap = intersection_overlap(
                                np.array(
                                    [
                                        ed_it["data"][0, :] // dataset.prediction_resolution,
                                        (ed_it["data"][0, :] + ed_it["data"][1, :]) // dataset.prediction_resolution,
                                    ]
                                ).T,
                    ...
                    for es_key, es_it in dataset.events_select[record].items():
                        if predictions_:
                            overlap = intersection_overlap(
                                np.array(
                                    [
                                        es_it["data"][0, :] // dataset.prediction_resolution,
                                        (es_it["data"][0, :] + es_it["data"][1, :]) // dataset.prediction_resolution,
                                    ]
                                ).T,
                                np.array(predictions_),
                            )
...
def predict_dataset_semantic(model, dataset, save_prediction_path=""):
    ...
    predictions = {record: {event: [] for event in events} for record in dataset.records}
    targets = {record: {event: [] for event in events} for record in dataset.records}
    ...
    for record in dataset.records:
        ...
        for signals, start_indexes in dataset.get_record_batch(record):
            ...
            for prediction_window, start_index in zip(predictions_batch, start_indexes):
                ...
                localizations_ = list(
                    range(
                        int(start_index) // dataset.prediction_resolution,
            ...
            for prediction_window, start_index in zip(predictions_batch, start_indexes):
                ...
                for loc_ in localizations_:
                    if loc_ > localizations[-1]:
                        localizations += [loc_]
                        for class_num, event in enumerate(events):
                            predictions[record][event] += [
                                prediction_window[loc_ % dataset.predictions_per_window, class_num]
                            ]
                            targets[record][event] += [event_[loc_ % dataset.predictions_per_window, class_num]]
        max_index = [idr["max_index"] for idr in dataset.index_to_record if idr["record"] == record][0]
        assert len(localizations) - 1 == int(
            (max_index + dataset.window_size) / (dataset.fs * dataset.prediction_resolution)
        )
...
def train(
    ...
    model.compile(
        loss=loss_functions[loss["type"]](**loss["args"]),
        run_eagerly=False,
        optimizer=Adam(
            learning_rate=learning_rate, epsilon=1e-8
        ),  # optimizer.DistributedShampoo(learning_rate=learning_rate),
    ...
    # weights and biases:
    wandb.init(
        project=project,  #'SleepStagePrediction', #'SDB-detection',
        config=logging_dict,
        entity="madsol",
...
def get_callbacks(
    model_name, monitor, patience, reduce_patience, model_directory, tensorboard_directory, learning_rate=1e-3
):  # ,
    # training_log_directory):
    ...
    # learning rate
    earlystopping = EarlyStopping(monitor="val_" + monitor, mode="max", min_delta=0, patience=patience)
    ...
    tensorboard = TensorBoard(
        tensorboard_directory, write_graph=True, write_images=True, histogram_freq=10, profile_batch=100
    )
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch % self.epoch_freq == 0:
            ...
            for dataset_name, dataset_idx in self.dataset_names.items():
                ...
                for count, (dataset_names, records, indexes) in enumerate(
                    self.collect_in_batch(
            ...
            for dataset_name, dataset_idx in self.dataset_names.items():
                f1_rows = []
                for dataset_names, records, indexes in self.collect_in_batch(
                    dataset_name=dataset_name,
...
def get_model_activation(model):
    ...
    layer_outputs_cnn_temp = [
        layer.output for layer in model.layers if layer.name[:4] == "conv" and layer.input.name[:4] == "spat"
    ]
