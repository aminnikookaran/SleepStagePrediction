...
class DatasetGenerator(Sequence):
    ...
    def __init__(
        ...
        if use_mask:
            ...
            self.events_discard_format += [
                {"name": sf["h5_path"] + "_mask", "h5_path": sf["h5_path"] + "_mask", "extend": 0}
        ...
        data = Parallel(n_jobs=n_jobs, prefer="threads")(
            delayed(self.load_data)(filename="{}/{}".format(h5_directory, record), signals=signals_format)
        ...
        for record, data in zip(records, data):
            ...
            signal_durations = [
                int(data[signal_name].shape[0] / signal_format["fs_post"])
            ...
            number_of_windows = max(
                1, int((record_duration_clipped - self.window) // (self.window * (1 - self.overlap)) + 1)
            )
    ...
    def __getitem__(self, idx):
        ...
        signal_batch = np.zeros((self.batch_size, int(self.window * self.fsTime), self.nSpace, self.nChannels)).astype(
            "float32"
        )
        event_batch = []
        ...
        for num, idx_ in enumerate(range(idx * self.batch_size, (idx + 1) * self.batch_size)):
            ...
            for signal_name, signal_format in self.signals_format.items():
                ...
                if signal_format["batch_normalization"]:
                    signal = normalizers[signal_format["batch_normalization"]["type"]](
                        signal, **signal_format["batch_normalization"]["args"]
                    )
            ...
            # events:
            event_batch += [
                self.get_events(record=self.index_to_record[idx_]["record"], index=self.index_to_record[idx_]["index"])
            ]
    ...
    def signal_model_prep(self, signal_batch):
        ...
        if isinstance(self.model_format, dict):
            ...
            for models in self.model_format:
                model_input += [
                    np.concatenate([signal_batch[signal_name] for signal_name in models["signals"]], axis=-1)
                ]
        return model_input
    ...
    def get_signals(self, record, signal_name, index):
        ...
        if self.load_signal_in_RAM:
            signal_data = self.signals[record]["data"][signal_name][
                int(index * fs) : int((index + self.window) * fs), :
            ]
        else:
            data = self.load_data(filename="{}/{}".format(self.h5_directory, record), signals=self.signals_format)
    ...
    def get_events(self, record, index):
        ...
        if self.use_mask:
            ...
            masks = self.get_events_sample(
                index=index, events=self.events_discard[record], minimum_overlap=0, any_overlap=True
            )
            mask_any = np.sum(masks, axis=-1) > 0
    ...
    def get_auxiliary_signal(self, record, index):
        data, fs = get_h5_auxiliary(
            filename="{}\\{}\\{}".format(self.h5_directory, self.h5_directory_auxiliary, record),
    ...
    def get_events_sample(self, index, events, minimum_overlap, any_overlap=True, minimum_duration=10):
        ...
        for event_name, event in events.items():
                ...
                for valid_index in valid_indexes:
                    if (durations[valid_index]) > minimum_duration:
                        if (valid_index in valid_starts_index) and (valid_index in valid_stops_index):
                            events_data.append(
                                (float(starts_relative[valid_index]), float(stops_relative[valid_index]))
                            )
            ...
            events_semantic[:, count] = semantic_formating(
                output_size=self.predictions_per_window,
                events=events_data,
    ...
    def get_record_events(self, record):
        ...
        for event_data in self.events[record].values():
            events[event_data["label"]].extend(
                [[start, (start + duration)] for start, duration in event_data["data"].transpose().tolist()]
            )
    ...
    def get_record_batch_(self, record):
        ...
        signal_batch = {
            signal_name: np.zeros(
                [self.batch_size, int(self.window * signal_format["fs_post"])] + signal_format["dimensions"]
            )
            for signal_name, signal_format in self.signals_format.items()
        }
        ...
        for record_index in record_indexes:
            for signal_name, signal_format in self.signals_format.items():
                signal = self.get_signals(
                    record=record_index["record"], signal_name=signal_name, index=record_index["index"]
                )
                if signal_format["batch_normalization"]:
                    signal = normalizers[signal_format["batch_normalization"]["type"]](
                        signal, **signal_format["batch_normalization"]["args"]
                ...
                signal_batch[signal_name][counter, : signal.shape[0], :] = np.reshape(
                    signal, [signal.shape[0]] + signal_format["dimensions"]
                )
            ...
            if counter == self.batch_size:
                ...
                if isinstance(self.model_format, dict):
                    sig = np.concatenate(
                        [signal_batch[signal_name] for signal_name in self.model_format["signals"]], axis=-1
                    )
                    check_inf_nan(sig)
                    model_input += [sig]
                else:
    ...
    def get_record_events_by_label(self, record, event_label, extent_back=0, extent_forward=0):
        ...
        data = get_h5_events(filename="{}/{}".format(self.h5_directory, record), event=event_label)
    ...
    def get_sample(self, record, index):
        ...
        events = self.get_events_sample(
            record=record, index=index, events=self.events[record], minimum_overlap=self.minimum_overlap
        )
        masks = self.get_events_sample(
            record=record, index=index, events=self.events_discard[record], minimum_overlap=0, any_overlap=True
        )
...
class BalancedDatasetGenerator(DatasetGenerator):
    ...
    def extract_balanced_data(self):
        ...
        event = self.random.choice(
            (self.number_of_classes + 1), p=self.event_probabilities + [1 - sum(self.event_probabilities)]
        )
