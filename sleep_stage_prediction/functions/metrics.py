...
def calculate_AUCRC(lower_bound=0.01, upper_bound=0.99, num_samples=100):
    ...
    def AUPRC(y_true, y_pred):
        ...
        for n, threshold in enumerate(thresholds):
            ...
            y_pred_pos = K.round(K.clip((y_pred + (1 - threshold) - 0.5), K.epsilon(), 1.0 - K.epsilon()))
...
def optimized_f1(lower_bound=0.2, upper_bound=0.8, num_samples=100, beta=1):
...
def optimized_f12(num_classes, lower_bound=0.2, upper_bound=0.8, num_samples=100, beta=1):
...
def binary_accuracy():
    def binary_accuracy(y_true, y_pred):
        ...
        y_pred_pos = K.round(K.clip(y_pred, K.epsilon(), 1.0 - K.epsilon()))
        y_pred_neg = 1 - y_pred_pos
...
def cohens_kappa1(num_classes, lower_bound=0.2, upper_bound=0.8, num_samples=100, beta=1):
    thresholds = np.linspace(start=lower_bound, stop=upper_bound, num=num_samples)
...
def get_false_positive_events(target_loc, output_loc, min_iou=0.5):
    ...
    if len(target_loc) == 0 and len(output_loc) > 0:
        ...
        iou = jaccard_overlap(np.array(target_loc), np.array(output_loc))
        max_iou = iou.max(0)
