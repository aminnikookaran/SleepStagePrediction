...
def relative_start_by_intersection(localizations_a, localizations_b):
    ...
    # relative start
    rel_start = np.repeat(np.expand_dims(localizations_b[:, 0], axis=0), A, axis=0) - np.repeat(
        np.expand_dims(localizations_a[:, 0], axis=1), B, axis=1
    )
    rel_stop = np.repeat(np.expand_dims(localizations_b[:, 1], axis=0), A, axis=0) - np.repeat(
        np.expand_dims(localizations_a[:, 1], axis=1), B, axis=1
    )
